...
// 1. 食事選択オーダー作成 & 別ホストへ送信 API (内部用)
router.post('/meal-orders', async (req, res) => {
  try {
    const { targetPersonId, mealType, timing, stapleFood, sideDish, allergies, remarks, remoteHostUrl, scheduledAt } = req.body;

    if (!mealType || !mealType.trim()) {
      if (req.accepts('html')) {
        return res.send('<script>alert("食形態・食種を選択してください。"); window.history.back();</script>');
      }
      return res.status(400).json({ success: false, message: '食形態・食種は必須です。' });
    }
...
・指示・備考: ${remarks ? remarks.trim() : '特記事項なし'}`;
    ...
    const title = `🍱 食事指示: ${mealType.trim()} (${targetDoc ? targetDoc.name : '全体'})`;
    ...
    if (remoteHostUrl && remoteHostUrl.trim() !== '') {
      ...
      if (!targetUrl.endsWith('/api/meal-orders/receive')) {
        targetUrl = targetUrl.replace(/\/+$/, '') + '/api/meal-orders/receive';
      }
      ...
      try {
        console.log(`[別ホスト食事オーダー送信] 送信先: ${targetUrl}`);
        ...
        if (response.ok) {
          ...
          console.log(`[別ホスト食事オーダー送信成功]`, resData);
        } else {
        ...
        console.error(`[別ホスト食事オーダー送信例外]`, sendErr.message);
      }
    ...
    res.status(201).json({ success: true, message: '食事選択オーダーを発行しました。', article: newArticle });

  } catch (err) {
    console.error('食事オーダー登録エラー:', err);
    res.status(500).send('食事オーダー登録エラー: ' + err.message);
  }
});

// 2. 外部ホストからの食事オーダー受信 API (公開・認証不要エンドポイント)
router.post('/meal-orders/receive', async (req, res) => {
  try {
    const {
      isTest,
      targetCustomId,
    ...
    console.log(`[外部食事オーダー受信] 送信元: ${senderHost || '未知ホスト'}, 担当: ${senderUser || '不明'}, 食種: ${mealType}, テストフラグ: ${isTest}`);

    // 💡 疎通テストの場合は記事として保存せず、テスト成功応答を返して捨てる
    if (isTest) {
      console.log(`[疎通テスト受信] 送信元: ${senderHost} からの食事オーダー接続テストを確認。記事には保存せず捨てます。`);
      return res.json({
        success: true,
        isTest: true,
    ...
    if (!mealType) {
      return res.status(400).json({ success: false, message: '食種・食形態データが必要です。' });
    }
...
・食形態・食種: ${mealType}
・提供区分: ${timing || '毎食'}
・主食/副食: 主食: ${stapleFood || '標準'} / 副食: ${sideDish || '標準'}
・アレルギー・禁止食: ${allergies || 'なし'}
・指示・備考: ${remarks || 'なし'}`;

    const title = `🍱 外部受信食事: ${mealType} (${targetDoc ? targetDoc.name : '外部対象'})`;
    ...
    res.status(201).json({
      success: true,
      message: '食事選択オーダーを正常に受信し記録しました。',
      articleId: receivedArticle._id
    });

  } catch (err) {
    console.error('食事オーダー受信処理エラー:', err);
    res.status(500).json({ success: false, message: '受信処理エラー: ' + err.message });
  }
});

// 3. 食事オーダー用接続疎通テスト API (記事保存なしでテスト確認)
router.post('/meal-orders/test-connection', async (req, res) => {
  try {
    ...
    if (!url.endsWith('/api/meal-orders/receive')) {
      url = url.replace(/\/+$/, '') + '/api/meal-orders/receive';
    }
